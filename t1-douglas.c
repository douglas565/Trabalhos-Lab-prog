...
int acha_num(int t, int v[t], int n)
{
  for (int i = 0; i < t; i++)
  {
    if (v[i] == n)
    {
      return i; // Retorna a posição quando o número é encontrado
    }
  }
...
void remove_pos(int t, int v[t], int p)
{
  if (p >= 0 && p < t)
  {
    int valorRemovido = v[p]; // Armazena o valor removido

    for (int i = p; i < t - 1; i++)
    {
      v[i] = v[i + 1]; // Move os elementos para a esquerda
    }
    v[t - 1] = valorRemovido; // Coloca o valor removido no final
